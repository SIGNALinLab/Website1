...
for g in d['groups'].values():
 for p in g['records']:
  ...
  if any(k in p['media'] for k in ['preview','previewWidth','previewHeight']):errors.append('Project still references derived preview media: '+p['id'])
  if not all(isinstance(p['media'].get(k),int) and p['media'][k]>0 for k in ['width','height']):errors.append('Missing project media dimensions for '+p['id'])

projects_js=(root/'assets/js/projects.js').read_text()
if "<img src=\"${esc(p.media.src)}\" width=\"${esc(p.media.width||'')}\" height=\"${esc(p.media.height||'')}\"" not in projects_js:errors.append('Landing project cards are not using original project media')
