...
function profileIcon(kind){
  const icons={
    ...
    recommendation:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="10.25" fill="currentColor"/><path d="M7.1 5.65h6.25l3.55 3.55v6.25c0 .76-.62 1.38-1.38 1.38H7.1c-.76 0-1.38-.62-1.38-1.38V7.03c0-.76.62-1.38 1.38-1.38Z" fill="#fff"/><path d="M13.35 5.65V9.2h3.55" fill="none" stroke="currentColor" stroke-width="1.05" stroke-linejoin="round"/><path d="M8.1 10.9h5.15M8.1 12.9h4.25M8.1 14.9h3.1" fill="none" stroke="currentColor" stroke-width="1.05" stroke-linecap="round"/><circle cx="16.55" cy="16.35" r="4.05" fill="currentColor" stroke="#fff" stroke-width="1.15"/><path d="M15.22 17.8v-2.56c0-.27.11-.53.3-.72l.9-.9c.24-.24.39-.55.43-.89l.05-.43c.03-.28.27-.49.55-.49.4 0 .7.37.61.76l-.21.91h1.26c.53 0 .9.5.77 1.01l-.61 2.41c-.09.36-.42.61-.79.61h-1.44a2 2 0 0 1-.83-.18l-.99-.45Z" fill="#fff"/><path d="M14.35 15.05h.87v2.78h-.87Z" fill="#fff"/></svg>'
  };
...
function mediaRecommendationControl(p){
  ...
  return `<a class="profile-media-recommendation-icon${isDemo?' is-demo':''}" href="${contentEsc(url)}" download="${contentEsc(filename)}" aria-label="Download ${contentEsc(label)} for ${contentEsc(p.name)}" title="${contentEsc(label)}">${profileIcon('recommendation')}</a>`;
}
function profileMediaRail(p,group,portraitHtml){
  ...
  const recommendation=(group==='graduate'||group==='undergraduate'||group==='alumni')?mediaRecommendationControl(p):'';
