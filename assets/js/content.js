...
function profileIcon(kind,group=''){
  ...
  const icons={
    ...
    linkedin:'<img src="data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%271.465 1.465 21.07 21.07%27%3E%3Crect x=%273%27 y=%273%27 width=%2718%27 height=%2718%27 rx=%272.2%27 fill=%27%230A66C2%27/%3E%3Ccircle cx=%278%27 cy=%279%27 r=%271.35%27 fill=%27white%27/%3E%3Cpath fill=%27white%27 d=%27M6.8 11.1h2.4v6.1H6.8zm3.9 0H13v.84h.03c.32-.6 1.1-1.23 2.26-1.23 2.42 0 2.87 1.55 2.87 3.57v2.92h-2.4v-2.59c0-.62-.01-1.42-.94-1.42-.94 0-1.09.68-1.09 1.37v2.64h-2.4z%27/%3E%3C/svg%3E" alt="" style="display:block;width:74%;height:74%;margin:13%;object-fit:contain">',
    x:'<img src="data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%272.625 2.625 18.75 18.75%27%3E%3Cpath fill=%27%23111111%27 d=%27M5.2 4h3.72l3.72 5.02L17.06 4H19l-5.47 6.29L19.2 20h-3.71l-4.12-5.56L6.48 20H4.53l5.95-6.84zM8.03 5.5l8.18 13h1.18L9.21 5.5z%27/%3E%3C/svg%3E" alt="" style="display:block;width:74%;height:74%;margin:13%;object-fit:contain">',
...
function profileMediaRail(p,group,portraitHtml){
  ...
  const linkedin=contentSafeUrl(p.linkedin)?`<a class="profile-media-cv-icon" href="${contentEsc(contentSafeUrl(p.linkedin))}" target="_blank" rel="noopener" aria-label="LinkedIn for ${contentEsc(p.name)}" title="LinkedIn">${profileIcon('linkedin')}</a>`:'';
  const x=contentSafeUrl(p.x)?`<a class="profile-media-cv-icon" href="${contentEsc(contentSafeUrl(p.x))}" target="_blank" rel="noopener" aria-label="X for ${contentEsc(p.name)}" title="X">${profileIcon('x')}</a>`:'';
  const social=linkedin||x;
  const actionStyle=social?' style="display:flex;flex-direction:row;align-items:center;justify-content:center;width:max-content;max-width:100%;gap:.10rem"':'';
  const actions=(cv||recommendation||social)?`<div class="profile-media-actions"${actionStyle}>${cv}${recommendation}${linkedin}${x}</div>`:'';
