...
function renderHeader(){
  ...
  const syncHeaderBorder=()=>h.classList.toggle('is-scrolled',window.scrollY>0);
  syncHeaderBorder();
  window.addEventListener('scroll',syncHeaderBorder,{passive:true});
  if(page==='people'){
    const jumpSection=document.querySelector('.people-jump-section');
    const jumpNav=document.querySelector('.people-jump-nav');
    if(jumpSection&&jumpNav){
      const syncPeopleJumpState=()=>{
        const stickyTop=parseFloat(getComputedStyle(jumpSection).top)||0;
        const isStuck=window.scrollY>0&&jumpSection.getBoundingClientRect().top<=stickyTop+.5;
        jumpNav.classList.toggle('is-stuck',isStuck);
      };
      syncPeopleJumpState();
      window.addEventListener('scroll',syncPeopleJumpState,{passive:true});
      window.addEventListener('resize',syncPeopleJumpState);
    }
  }
}
