...
This optional backend supports the native SIGNAL Lab application and contact forms while the public website remains static on GitHub Pages.
...
4. Run `initializeSignalSheets()` once and authorize it. This initializes only the `Applications` and `Contact` tabs and stores the bound spreadsheet ID in Apps Script Script Properties for reliable Web-app writes.
...
- The backend accepts only `application` and `contact` form types.
...
- Writes use a script lock to reduce concurrent-write collisions.
